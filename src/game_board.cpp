...
void GameBoard::RenderBackGroundGrid() {
    ...
    for (int i = 1; i <= 3; ++i) {
        ...
        mvaddch(y + (this->height + 1) * i, x + HORIZONTAL_SIDE_LENGTH,
                ACS_RTEE);
    ...
    for (int i = 1; i <= 3; ++i) {
        for (int j = 1; j <= 3; ++j) {
            mvaddch(y + (this->height + 1) * i, x + (this->width + 1) * j,
                    ACS_PLUS);
        }
    }
...
void GameBoard::ReflectGameBoardState() {
    ...
    for (int i = 0; i < TO_BE_RENDERED; ++i) {
        for (int j = 0; j < TO_BE_RENDERED; ++j) {
            ...
            mvaddstr(y + margin(this->height) + j * (this->height + 1),
                     x + margin(this->width) + (this->width + 1) * i,
                     str.c_str());
        }
    }
}